...
- [gdpar](https://github.com/IsadoreNabi/gdpar) - **This package provides a unified framework for individual-level parameter estimation via reference anchoring, decomposing each unit's parameter as a population reference plus a structured, covariate-driven deviation**. ([GitHub](https://github.com/IsadoreNabi/gdpar)). It implements the Additive–Multiplicative–Modulated (AMM) canonical form — θ_i = θ_ref + a(x_i) + b(x_i)·θ_ref + W(θ_ref)·x_i — estimated by hierarchical Bayesian Hamiltonian Monte Carlo (via cmdstanr) in both full-Bayes and empirical-Bayes regimes, with closed-form identifiability conditions verified before fitting. Designed for distributional regression, it anchors every parameter slot (location, scale, shape, tail index, zero-inflation) across thirteen response families — from Gaussian, Poisson, negative-binomial, Beta, Gamma, Student-t, Tweedie and lognormal to zero-inflated and hurdle count models — and ships a causal T-learner bridge for individual treatment effects (CATE/ITE) benchmarked against external meta-learners, an opt-in geometry-adaptive sampler that climbs from Euclidean to Riemannian, sub-Riemannian and Finslerian metrics under a certifying orchestrator, and dependence-robust inference (block bootstrap with data-driven block lengths, Moran's I) that keeps uncertainty valid under temporal or spatial dependence, heavy tails, and severe group imbalance.
